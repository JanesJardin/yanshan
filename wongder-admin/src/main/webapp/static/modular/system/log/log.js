...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(Wongder.ctxPath + "/log/detail/" + this.seItem.id, function (data) {
            Wongder.infoDetail("日志详情", data.regularMessage);
        }, function (data) {
            Wongder.error("获取详情失败!");
        });
        ajax.start();
    }
};
...
OptLog.delLog = function () {
    Wongder.confirm("是否清空所有日志?",function(){
        var ajax = Wongder.baseAjax("/log/delLog","清空日志");
        ajax.start();
