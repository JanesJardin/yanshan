...
Dept.openAddDept = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Wongder.ctxPath + '/dept/dept_add'
    });
...
Dept.openDeptDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Wongder.ctxPath + '/dept/dept_update/' + Dept.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Dept.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(Wongder.ctxPath + "/dept/delete", function () {
        ...
        Wongder.confirm("是否刪除该部门?", operation);
    }
};
